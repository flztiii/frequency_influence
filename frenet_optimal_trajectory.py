...
def main():
    ...
    # 每一次更新所花费时间
    print('input replanning interval(seconds)(0s-2s):')
    gap = int(float(input()) * 10)
    if not (gap > 0 and gap < 21):
        print('input time is wrong')
        exit()
    # 开始进行规划
    while True:
    ...
    plt.title('Replanning interval is ' + str(gap * 0.1) + ' secondes', fontsize = 16)
    plt.ylabel('curvature[1/rad]', fontsize = 14)
